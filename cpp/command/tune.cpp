...
int MainCmds::tuner(const vector<string>& args) {
  ...
  try {
    ...
    TCLAP::ValueArg<string> outputFileArg(
      "", "output", "Filename to output tuning configration to", false, string(), "FILE");
    ...
    TCLAP::ValueArg<int> nnXLenArg(
      "", "xsize", "Width of board to tune for", false, OpenCLTuner::DEFAULT_X_SIZE, "INT");
    TCLAP::ValueArg<int> nnYLenArg(
      "", "ysize", "Height of board to tune for", false, OpenCLTuner::DEFAULT_Y_SIZE, "INT");
    TCLAP::ValueArg<string> testFP16Arg(
      "", "testFP16", "Test FP16? true|false|auto (default auto)", false, "auto", "BOOL_OR_AUTO");
    TCLAP::ValueArg<string> testFP16StorageArg(
      "", "testFP16Storage", "Test FP16 storage? true|false|auto (default auto)", false, "auto", "BOOL_OR_AUTO");
    TCLAP::ValueArg<string> testFP16ComputeArg(
      "", "testFP16Compute", "Test FP16 compute? true|false|auto (default auto)", false, "auto", "BOOL_OR_AUTO");
    TCLAP::ValueArg<string> testFP16TensorCoresArg(
      "",
      "testFP16TensorCores",
    ...
    TCLAP::ValueArg<int> batchSizeArg(
      "", "batchsize", "Batch size to tune for", false, OpenCLTuner::DEFAULT_BATCH_SIZE, "INT");
  ...
  for(int i = 0; i < gpuIdxs.size(); i++) {
    ...
    if(contains(alreadyTunedNames, device->info.name)) {
      cout << "Skipping tuning " << gpuIdx << " due to same name as an earlier tuned GPU: " << device->info.name
           << endl;
      continue;
    }
