...
struct CmdCtx {
  ...
  VulkanProfiler::DispatchProfiler* profiler = nullptr;
};

// LaunchProfile
//
// Kernel-specific dispatch semantics that are not workgroup dimensions and so
// don't belong in localSizeX/localSizeY. build() stamps the matching variant on
// the ComputeKernel as it specializes the pipeline; dispatch() reads it back to
// recompute the launch grid or binding shape without re-passing those values as
// parameters. The DOT2 GEMM tile is the canonical case: BM/BN are tunable spec
// constants, so dispatch() needs them to compute the grid (divUp(M,BM),
// divUp(N,BN)); AttentionAccelQ similarly carries the split-K chunk count for
// the z-grid, and NhwcChannelTile the channels-per-invocation. Cross-layer
// readers (e.g. the split-K attention resolve in vulkanlayers.cpp) also use it.
...
struct ComputeKernel {
  ...
  // breakdown when KATAGO_VULKAN_PROFILE_KERNELS=1 and for the kernel-name trace
  // when KATAGO_VULKAN_TRACE_KERNELS=1. Empty means unnamed.
  ...
  // If ctx.profiler is non-null and active, records the dispatch: a (start, end)
  // timestamp pair on the profiler's query pool when timing is enabled, or just
  // the kernel name when trace mode is on.
  void dispatch(
    const CmdCtx& ctx,
...
class TunableKernel {
 public:
  ...
  // The VulkanTuner::TunedKernel validity bit this kernel's tuning records.
  // Pure virtual so a newly added micro-kernel that the tuner's driver loop
  // must handle is a compile error rather than a runtime throw, and so the
  // driver can key off the bit directly instead of comparing name() strings.
  // Return 0 for kernels the driver loop does not tune directly (the
  // accelerated GEMM/conv/attention variants, which have their own drivers).
  virtual int64_t tunedBit() const = 0;
...
// Kernel-flavor validity helper for VulkanTuneParams.
...
namespace VulkanKernels {
  ...
  // K per shared-tile step of the DOT2 shaders. Shader-side `#define BK 32` in
  ...
  // The strided DOT2 GEMM shaders select B-tile layout through the PACKED_B spec
  // constant: nonzero reads the contiguous pre-packed tile (packStridedGemmBWeights),
  // zero uses the slower pack4-N scatter path. The pre-packed layout has always
  // measured faster, so every caller passes this value. Kept as a parameter (rather
  // than folded into the shaders) so it continues to mirror the PACKED_B constant_id.
  constexpr int32_t DOT2_PACKED_B = 1;

  // Shared-memory footprint of every DOT2 GEMM shader, in bytes.
  //
  // All four (winograd_gemm_dot2_accf{32,16}.glsl,
  // gemm_strided_dot2_accf{32,16}_nhwc.glsl) declare the same pair of tiles:
  //
  ...
  //
  // BK is the compile-time `#define BK 32` (== DOT2_BK), not a spec constant, so
  // only BM/BN vary per tuned config. The +4 is the bank-conflict pad.
  //
  // The tuner screens candidates against maxComputeSharedMemorySize with this
  // before compiling them: a tile that overruns shared memory can hang the GPU
  // rather than fail cleanly, which loses the device for the rest of the process.
  constexpr size_t DOT2_SHMEM_PAD = 4;
  inline size_t dot2SharedBytes(int32_t bm, int32_t bn) {
    if(bm <= 0 || bn <= 0)
      return 0;
    const size_t stride = (size_t)DOT2_BK + DOT2_SHMEM_PAD;
    return ((size_t)bm + (size_t)bn) * stride * sizeof(uint16_t);
  }
  ...
  struct ScaleBiasMaskActNhwc {
    ...
    build(VkDevice device, VkPipelineCache cache, bool fp16, int activation, bool useVec4, bool hasDynamicBias = false);
  ...
  struct NchwToNhwc {
    ...
    static Kernels build(VkDevice device, VkPipelineCache cache, bool fp16, const VulkanTuneParams& tuneParams);
    static void
    dispatch(const CmdCtx& ctx, const Kernels& kernels, VulkanBuffer* input, VulkanBuffer* output, const PC& pc);
  };

  // Whether a (smallTile, largeTile, tileCrossover) triple is a usable tiled
  // NCHW->NHWC configuration (tile dims power-of-two and crossover ordering sane).
  ...
  struct NchwToNhwcTuner : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_NCHW_TO_NHWC; }
  ...
  struct WinogradTransformNhwc : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_TRANSFORM; }
  ...
  // Element-wise add of two equal-length buffers (acc += value). Used to fold a
  // residual branch back into the trunk when the conv couldn't fuse the add.
  struct AddPointwise {
    struct PC {
      int numElements;
    };
  ...
  struct WinogradUntransformNhwc : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_UNTRANSFORM; }
    int topK() const override { return 3; }
    static ArrayView<TunableParam> sharedParams();
    ArrayView<TunableParam> params() const override;
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
  ...
  struct WinogradGemm : public TunableKernel {
    ...
    static ComputeKernel
    build(VkDevice device, VkPipelineCache cache, bool fp16, const VulkanTuneParams& cfg, int32_t k);
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_TILED; }
    int topK() const override { return 3; }
    static ArrayView<TunableParam> sharedParams();
  ...
  struct GemmStridedTiledNhwc : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_GEMM_STRIDED_TILED; }
    static ArrayView<TunableParam> sharedParams();
  ...
  // FP16 only. Uses ggml-style subgroup tiling with f16vec2 shared memory and v_dot2_f32_f16.
  ...
  struct WinogradGemmDot2 : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_DOT2; }
    int topK() const override { return 3; }
    static ArrayView<TunableParam> sharedParams();
    ...
    ArrayView<TunableParam> params() const override;
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override { return (uint32_t)cfg.dot2WorkgroupSize; }
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int iters) const override;
    double estimatedFlopsPerDispatch(const VulkanTuneParams& cfg) const override;
  ...
  struct WinogradGemmDot2AccF16 : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_DOT2_ACCF16; }
    int topK() const override { return 3; }
    static ArrayView<TunableParam> sharedParams();
    static LayerPaddingContract layerPaddingContract(const VulkanTuneParams& cfg) {
    ...
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override {
      return (uint32_t)cfg.dot2AccF16WorkgroupSize;
    }
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int iters) const override;
    double estimatedFlopsPerDispatch(const VulkanTuneParams& cfg) const override;
  ...
  struct WinogradGemmCoopmat1 : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_COOPMAT1; }
    int topK() const override { return 3; }
    static ArrayView<TunableParam> sharedParams();
    ...
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override {
      return (uint32_t)cfg.coopmat1WorkgroupSize;
    }
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int iters) const override;
    double estimatedFlopsPerDispatch(const VulkanTuneParams& cfg) const override;
  ...
  // Common coopmat1 geometry predicate shared by the Winograd and strided
  // variants. Each subgroup owns a (SGM/TM)x(SGN/TN) grid of TMxTN accumulator
  // fragments and contracts BK in TK steps. TM/TN/TK must match a device-reported
  // shape (membership is checked by bench(), which has the device shape list).
  inline bool coopmat1GemmConfigSupported(
    int32_t workgroupSize,
    int32_t bm,
    int32_t bn,
    int32_t bk,
    ...
    int32_t tk,
    int32_t subgroupSize) {
    if(workgroupSize <= 0 || bm <= 0 || bn <= 0 || bk <= 0 || sgm <= 0 || sgn <= 0)
      return false;
    ...
    if(sgm % tm != 0 || sgn % tn != 0)
      return false;  // fragment grid tiles the SGM×SGN subgroup tile exactly
    if(bk % tk != 0)
      return false;  // K contracted in TK steps over the BK slab
    // One subgroup per SGM×SGN region: subgroup_c = subgroup_i/(BM/SGM).
  ...
  struct WinogradGemmCoopmat1AccF16 : public TunableKernel {
    ...
    static bool isConfigSupported(
      ...
      int32_t tk,
      int32_t subgroupSize) {
      return WinogradGemmCoopmat1::isConfigSupported(workgroupSize, bm, bn, bk, sgm, sgn, tm, tn, tk, subgroupSize);
    }
    static void dispatch(
      const CmdCtx& ctx,
      const ComputeKernel& kernel,
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_COOPMAT1_ACCF16; }
    int topK() const override { return 3; }
    static ArrayView<TunableParam> sharedParams();
    static LayerPaddingContract layerPaddingContract(const VulkanTuneParams& cfg) {
    ...
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override {
      return (uint32_t)cfg.coopmat1AccF16WorkgroupSize;
    }
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int iters) const override;
    double estimatedFlopsPerDispatch(const VulkanTuneParams& cfg) const override;
  ...
  struct GemmStridedCoopmat1Nhwc {
    ...
    static bool isConfigSupported(
      ...
      int32_t tk,
      int32_t subgroupSize) {
      if(!coopmat1GemmConfigSupported(workgroupSize, bm, bn, bk, sgm, sgn, tm, tn, tk, subgroupSize))
        return false;
      if(tm % 8 != 0 || sgm % 8 != 0)
        return false;  // packed half8 A coopMatLoad offsets must start on M/8 boundaries
      return bk % 8 == 0 && tk % 8 == 0;
    }
    static size_t sharedBytes(
      ...
      int32_t tn,
      int32_t subgroupSize) {
      const int32_t numSubgroups = subgroupSize > 0 ? workgroupSize / subgroupSize : 1;
      ...
      const size_t stageBytes = (size_t)std::max(1, numSubgroups) * tm * tn * sizeof(float);
  ...
  struct GemmStridedCoopmat1AccF16Nhwc {
    ...
    static size_t sharedBytes(
      ...
      const size_t aBytes = (size_t)bm * ((size_t)bk / 8 + 1) * sizeof(uint32_t) * 4;
      const size_t bBytes = (size_t)bk * ((size_t)bn + STRIDED_COOPMAT_PACKED_B_PAD_SCALARS) * sizeof(uint16_t);
      const size_t stageBytes = (size_t)std::max(1, numSubgroups) * tm * tn * sizeof(uint16_t);
      return aBytes + bBytes + stageBytes;
    }
    static ComputeKernel build(
      VkDevice device,
  ...
  struct GemmStridedDot2Nhwc {
    using PC = GemmStridedPC;
    static constexpr bool kSingleWriterStore = true;
    // Shared-memory footprint: the [BM][BK+4] + [BN][BK+4] fp16 tile pair the
    // shader declares (BK is its compile-time #define, == DOT2_BK). Exposed so
    // the tuner can screen a tile against maxComputeSharedMemorySize before
    // compiling it -- an overrun can hang the GPU rather than fail cleanly.
    static size_t sharedBytes(int32_t bm, int32_t bn) { return dot2SharedBytes(bm, bn); }
    static bool isConfigSupported(
      int32_t workgroupSize,
      int32_t bm,
      int32_t bn,
  ...
  struct WinogradGemmCoopmat2 : public TunableKernel {
    using PC = WinogradGemm::PC;
    static ComputeKernel build(VkDevice device, VkPipelineCache cache, const VulkanTuneParams& cfg, int32_t k);
    static bool isConfigSupported(int32_t workgroupSize, int32_t bm, int32_t bn, int32_t bk);
    static void dispatch(
      const CmdCtx& ctx,
      const ComputeKernel& kernel,
      VulkanBuffer* A,
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_COOPMAT2; }
    int topK() const override { return 3; }
    static ArrayView<TunableParam> sharedParams();
    static LayerPaddingContract layerPaddingContract(const VulkanTuneParams& cfg) {
    ...
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override {
      return (uint32_t)cfg.coopmat2WorkgroupSize;
    }
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int iters) const override;
    double estimatedFlopsPerDispatch(const VulkanTuneParams& cfg) const override;
  ...
  struct WinogradGemmCoopmat2AccF16 : public TunableKernel {
    ...
    static ComputeKernel build(VkDevice device, VkPipelineCache cache, const VulkanTuneParams& cfg, int32_t k);
    static bool isConfigSupported(int32_t workgroupSize, int32_t bm, int32_t bn, int32_t bk) {
      return WinogradGemmCoopmat2::isConfigSupported(workgroupSize, bm, bn, bk);
    }
    static void dispatch(
      const CmdCtx& ctx,
      const ComputeKernel& kernel,
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_WINOGRAD_COOPMAT2_ACCF16; }
    int topK() const override { return 3; }
    static ArrayView<TunableParam> sharedParams();
    static LayerPaddingContract layerPaddingContract(const VulkanTuneParams& cfg) {
    ...
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override {
      return (uint32_t)cfg.coopmat2AccF16WorkgroupSize;
    }
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int iters) const override;
    double estimatedFlopsPerDispatch(const VulkanTuneParams& cfg) const override;
  ...
  struct GemmStridedCoopmat2Nhwc {
    using PC = GemmStridedPC;
    // No shared memory: A/B/C stream through tensorLayoutNV +
    // coopMatLoadTensorNV at workgroup scope. The only shared cost is the
    // driver's workgroup-scope reservation (coopmat2ReservedSharedBytes), a
    // device property rather than a function of the tile, already checked
    // against the device limit in coopmat2TileSupported.
    static constexpr bool kSingleWriterStore = true;
    static bool isConfigSupported(int32_t workgroupSize, int32_t bm, int32_t bn, int32_t bk) {
  ...
  // Plain FP32 dense GEMM (C[M,N] += A[M,K] * B[K,N]), used by the fully-connected
  // (MatMul) layers where the strided/Winograd paths don't apply.
  struct GemmDirectFP32 : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_GEMM_DIRECT; }
    static ArrayView<TunableParam> sharedParams();
    ArrayView<TunableParam> params() const override;
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override {
  ...
  // Tiled flash-attention (NHWC). Specialised per (seqLen, qHeadDim, vHeadDim)
  // and tuned over attnNhwcBlockQ, attnNhwcBlockKV, attnNhwcQPerThread.
  struct AttentionTiled : public TunableKernel {
    ...
    AttentionTiled(
      ...
      bool learnableRope,
      int validTokens = -1);
    std::string name() const override { return "attnTiledNhwc"; }
    int64_t tunedBit() const override { return VulkanTuner::TUNED_ATTN_TILED; }
    int topK() const override { return 3; }
    ArrayView<TunableParam> params() const override;
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override { return (uint32_t)cfg.attnNhwcBlockQ; }
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int iters) const override;
    double estimatedFlopsPerDispatch(const VulkanTuneParams& cfg) const override;

   private:
    int problemBatchSize, problemNumTokens, problemValidTokens, problemHeadDim, problemVHeadDim, problemNumHeads,
      problemNumKVHeads;
    bool problemUseRope, problemLearnableRope;
  };

  // Stateless build()/dispatch() factory for the tiled flash-attention shader —
  // the non-coopmat fallback that the AttentionTiled TunableKernel wraps.
  ...
  struct AttentionCoopmat1Nhwc {
    ...
    static bool isConfigSupported(
      ...
      int headDim,
      int vHeadDim);
    // Validates the coopmat1 "mode" fields rather than the tile geometry:
    // the direct-KV specialization flags, the KV chunk counts (1 = single
    // pass), and the split-K batch cutoff. Tuner-local timing fields are
    // deliberately excluded; they only select among incumbents.
    static bool isModeConfigSupported(
      int directKV,
      int kvChunkCount,
      int splitKDirectKV,
      int splitKKVChunkCount,
      int splitKCutoffBatch);
    // User shared memory the shader consumes (Q/K/V tiles, RoPE tables, and the
    // optional maintenance1 score/prob reduction arrays); callers sum this against
    // the device's maxComputeSharedMemorySize before compiling.
    static size_t sharedBytes(
      int blockQ,
      int blockKV,
      int tk,
  ...
  struct AttentionCoopmat1Bench {
    ...
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int32_t kvChunkCount, int iters)
      const;
  ...
  struct AttentionCoopmat2AccF32Nhwc {
    ...
    static ComputeKernel build(
      ...
      bool useRope,
      bool learnableRope);
    static bool isConfigSupported(int workgroupSize, int blockQ, int blockKV, int headDim, int vHeadDim);
  ...
  // Tuner-only problem wrapper for benchmarking AttentionCoopmat2AccF32Nhwc
  // against the tiled baseline on identical randomized data.
  ...
  // Tuner-only problem wrapper for benchmarking AttentionDot2AccF32Nhwc.
  ...
  // SwiGLU gating: output = main * sigmoid(gate), applied per element over a
  // 4-wide vectorized buffer. Used by the transformer FFN block.
  struct SwiGLU : public TunableKernel {
    ...
    static ComputeKernel build(VkDevice device, VkPipelineCache cache, bool fp16, const VulkanTuneParams& cfg);
    static void dispatch(
      const CmdCtx& ctx,
      const ComputeKernel& kernel,
      VulkanBuffer* main,
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_SWIGLU; }
    static ArrayView<TunableParam> sharedParams();
    ArrayView<TunableParam> params() const override;
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
  ...
  struct GPoolReductionNhwc : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_GPOOL_NHWC; }
    static ArrayView<TunableParam> sharedParams();
    ArrayView<TunableParam> params() const override;
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override { return (uint32_t)cfg.gpoolNhwcXystride; }
    KernelBench bench(VulkanTuner::TuningContext& ctx, const VulkanTuneParams& cfg, int iters) const override;
    double estimatedFlopsPerDispatch(const VulkanTuneParams& cfg) const override;
  ...
  struct ValueHeadPoolNhwc : public TunableKernel {
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_VALUE_HEAD_POOL_NHWC; }
    static ArrayView<TunableParam> sharedParams();
    ArrayView<TunableParam> params() const override;
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override {
  ...
  // Per-position RMSNorm across channels. Spec constants: WG_XY_SIZE=2,
  // WG_C_SIZE=32, C_PER_THREAD=1, USE_FP16_STORAGE. Grid: gx = divUp(paddedSpatialSize, 2).
  struct TransformerRMSNormNhwc {
    ...
    static ComputeKernel
    build(VkDevice device, VkPipelineCache cache, bool fp16, bool useSubgroupVariant, uint32_t requiredSubgroupSize);
    static void dispatch(
      const CmdCtx& ctx,
      const ComputeKernel& kernel,
      VulkanBuffer* input,
  ...
  struct Conv3x3ImplicitGemmCoopmat2AccF32NhwcVec8 {
    ...
    static bool isConfigSupported(int32_t workgroupSize, int32_t bm, int32_t bn, int32_t bk) {
      ...
      // workgroupSize % (bk/8) == 0: the shared-A gather's lane mapping (kk8/mm) must
      // tile [0,bm) bijectively, exactly as the coopmat1 vec8 path requires.
      return bk % 8 == 0 && workgroupSize % (bk / 8) == 0;
    }
    ...
    static size_t sharedBytes(int32_t bm, int32_t bk) {
      return bm > 0 && bk > 0 ? (size_t)bm * ((size_t)bk + 8) * sizeof(uint16_t) : std::numeric_limits<size_t>::max();
    }
    static ComputeKernel build(
      VkDevice device,
      VkPipelineCache cache,
  ...
  // FP16-accumulator coopmat2 NHWC vec8 implicit-GEMM convolution; reuses the
  // coopmat2 A-staging sharedBytes from the FP32 variant.
  ...
  struct SpatialRMSNormNhwc : public TunableKernel {
    ...
    struct Pass1PC {
      int numChannels, paddedSpatialSize, tilesPerGroup, numSpatialWorkgroups;
    };
    struct Pass2PC {
    ...
    static std::array<ComputeKernel, 3> build(
      VkDevice device,
      VkPipelineCache cache,
      bool fp16,
      const VulkanTuneParams& cfg,
    ...
    int64_t tunedBit() const override { return VulkanTuner::TUNED_SPATIAL_RMSNORM_NHWC; }
    static ArrayView<TunableParam> sharedParams();
    ArrayView<TunableParam> params() const override;
    bool validate(const VulkanTuneParams& cfg, const VkPhysicalDeviceLimits& limits) const override;
    uint32_t workgroupThreads(const VulkanTuneParams& cfg) const override {
