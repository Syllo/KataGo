...
struct VulkanDeviceInfo {
  ...
  bool unfilteredSupportsGemmTiledFp16Compute;
  ...
  bool filterDisabledGemmTiledFp16Compute;
  ...
  // Deprecated tiled GEMM path using FP16 arithmetic/accumulation. Permanently
  // disabled because FP16 accumulation is too inaccurate for some models.
  bool supportsGemmTiledFp16Compute;
  // True when KATAGO_VULKAN_GEMM_VARIANTS or the legacy coopmat2-disable env
  // var constrained optional GEMM families/accumulators during discovery.
  bool gemmVariantFilterConfigured;
  // Canonical no-spaces signature for the current GEMM variant env filter.
