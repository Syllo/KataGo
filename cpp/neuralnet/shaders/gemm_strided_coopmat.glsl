// GLSL Compute Shader: Strided batched GEMM — FP16 with KHR cooperative matrix (coopmat1).
...
// and accumulated with coopMatMulAdd into an FP32 accumulator.
//
  ...
  [[unroll]] for(uint cmRow = 0; cmRow < cmsPerRow; cmRow++) {
    [[unroll]] for(uint cmCol = 0; cmCol < cmsPerCol; cmCol++) {
      ...
      if(mTileBase + TM <= uint(pc.M) && nTileBase + TN <= uint(pc.N_real)) {
        coopmat<float, gl_ScopeSubgroup, TM, TN, gl_MatrixUseAccumulator> outSum = sums[s];
        if(ADD_TO_OUTPUT != 0u) {
          ...
          outSum = outSum + coopmat<float, gl_ScopeSubgroup, TM, TN, gl_MatrixUseAccumulator>(addFrag);
        }
