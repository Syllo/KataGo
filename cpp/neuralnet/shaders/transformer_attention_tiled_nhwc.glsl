...
layout(binding = 0) readonly buffer QBuf { float data[]; } qBuf;
layout(binding = 1) readonly buffer KBuf { float data[]; } kBuf;
layout(binding = 2) readonly buffer VBuf { float data[]; } vBuf;
layout(binding = 3) writeonly buffer OutBuf { float data[]; } outBuf;
layout(binding = 4) readonly buffer MaskBuf { float data[]; } maskBuf;
layout(binding = 5) readonly buffer QBufH { float16_t data[]; } qBufH;
layout(binding = 6) readonly buffer KBufH { float16_t data[]; } kBufH;
layout(binding = 7) readonly buffer VBufH { float16_t data[]; } vBufH;
layout(binding = 8) writeonly buffer OutBufH { float16_t data[]; } outBufH;
layout(binding = 9) readonly buffer MaskBufH { float16_t data[]; } maskBufH;
layout(binding = 10) readonly buffer CosBuf { float data[]; } cosBuf;
layout(binding = 11) readonly buffer SinBuf { float data[]; } sinBuf;
...
int ropeTableIdx(int kvh, int pairIdx, int pos) {
  return LEARNABLE_ROPE == 1
    ? (kvh * (ATTN_HEAD_DIM / 2) + pairIdx) * SEQ_LEN_SPEC + pos
      ...
      if(qPos < SEQ_LEN_SPEC && qMask[qi] != 0.0) {
        for(int kv = 0; kv < ATTN_BLOCK_KV; kv++) {
          ...
          for(; d + 3 < ATTN_HEAD_DIM; d += 4) {
            vec4 qv = vec4(
              q[qi * ATTN_HEAD_DIM + d], q[qi * ATTN_HEAD_DIM + d + 1],
              q[qi * ATTN_HEAD_DIM + d + 2], q[qi * ATTN_HEAD_DIM + d + 3]);
            vec4 kvv = vec4(
              kTile[kv * ATTN_HEAD_DIM + d], kTile[kv * ATTN_HEAD_DIM + d + 1],
              kTile[kv * ATTN_HEAD_DIM + d + 2], kTile[kv * ATTN_HEAD_DIM + d + 3]);
            score += dot(qv, kvv);
          }
