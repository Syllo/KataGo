// GLSL Compute Shader: Winograd input tile transform (no fused BN), NHWC layout
...
//
// else -- the private-tile Winograd math and the packed-A output layout consumed by
// the (layout-neutral) WinogradGemm kernels -- is identical.
...
float loadInput(int idx) {
  if(USE_FP16_STORAGE == 1)
    return float(inputBufHalf.data[idx]);
  else
    return inputBuf.data[idx];
}
  ...
  for(int subY = 0; subY < INTILE_YSIZE; subY++) {
    ...
    for(int subX = 0; subX < INTILE_XSIZE; subX++) {
      ...
      if(validInput && y >= 0 && y < pc.nnYLen && x >= 0 && x < pc.nnXLen) {
        ...
        // NHWC: channel-innermost. The physical input tensor is stored with
        // the actual (unpadded) channel count as its stride -- only the
        // packed-A workspace this kernel writes to is padded -- so the
        // spatial position selects a row of numInChannels channels and ic
        // selects within that row.
        value = loadInput((n * pc.paddedSpatialSize + xy) * pc.numInChannels + ic);
      }
