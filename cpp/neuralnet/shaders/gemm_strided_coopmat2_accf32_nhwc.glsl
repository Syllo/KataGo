// Native-NHWC strided GEMM using NV cooperative matrix2 and FP32 accumulation.
...
layout(binding = 0) readonly buffer ABuf { float16_t data[]; } aBuf;
layout(binding = 1) readonly buffer BBuf { float16_t data[]; } bBuf;
layout(binding = 2) buffer CBuf { float16_t data[]; } cBuf;
...
void main() {
  ...
  if(ADD_TO_OUTPUT != 0u) {
    ...
    coopMatLoadTensorNV(c0, cBuf.data, cBase,
      sliceTensorLayoutNV(layoutCClamp, ir * BM, BM, ic * BN, BN));
    acc = coopmat<float, gl_ScopeWorkgroup, BM, BN, gl_MatrixUseAccumulator>(c0);
  }
  ...
  for(uint kb = 0u; kb < K; kb += BK) {
    ...
    if(full) {
      ...
      coopMatLoadTensorNV(b, bBuf.data, bTileBase,
        sliceTensorLayoutNV(layoutB, 0u, BK, 0u, BN));
    } else {
      coopMatLoadTensorNV(a, aBuf.data, aBase, sliceTensorLayoutNV(layoutAClamp, ir * BM, BM, kb, BK));
      coopMatLoadTensorNV(b, bBuf.data, bTileBase,
        sliceTensorLayoutNV(layoutBClamp, 0u, BK, 0u, BN));
    }
  ...
  if(ALIGNED != 0u && ic * BN + BN <= NReal)
    coopMatStoreTensorNV(matC, cBuf.data, cBase, sliceTensorLayoutNV(layoutC, ir * BM, BM, ic * BN, BN));
  else
    coopMatStoreTensorNV(matC, cBuf.data, cBase, sliceTensorLayoutNV(layoutCClamp, ir * BM, BM, ic * BN, BN));
}
