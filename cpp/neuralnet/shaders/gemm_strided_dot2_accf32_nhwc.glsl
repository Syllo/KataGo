// GLSL Compute Shader: Strided batched GEMM for native NHWC — FP16 with FP32 DOT2 accumulation
// (OpFDot2MixAcc32VALVE). For 1x1 convolutions and transformer projections.
...
// into an FP32 accumulator with two v_dot2 calls. Compute loop, register tiling and
// the A/C global-memory addressing differs (see gemm_strided_coopmat1_accf16_nhwc.glsl
// for the NHWC A/C layout this borrows).
//
...
uvec4 loadATail(uint aBase, uint gm, uint gk, uint K) {
  ...
  return uvec4(
    packHalfPair(h0, h1), packHalfPair(h2, h3), packHalfPair(h4, h5), packHalfPair(h6, h7));
}
  ...
  { \
    [[unroll]] for(uint s = 0; s < A_PER_THREAD_V8; s++) { \
      ...
      if(i < aCountV8) { \
        ...
        buf_a[base + 0u] = p0.x; buf_a[base + 1u] = p0.y; \
        buf_a[base + 2u] = p1.x; buf_a[base + 3u] = p1.y; \
        buf_a[base + 4u] = p2.x; buf_a[base + 5u] = p2.y; \
        buf_a[base + 6u] = p3.x; buf_a[base + 7u] = p3.y; \
      } \
    } \
  }
