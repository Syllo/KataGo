...
// FlashAttention-4 keeps the online-softmax origin fixed until the row
// maximum has moved by 8 in base-2 exponent space.  This avoids rescaling the
// entire output accumulator for every small increase in the running maximum,
// while keeping every unnormalised softmax weight <= 2^8.  Scores are kept in
// base-2 units so that exp2 maps to the GPU's native exponential operation.
    ...
    for(int qi = 0; qi < Q_PER_THREAD; qi++) {
      ...
      if(qPos < SEQ_LEN_SPEC && qMask[qi] != 0.0) {
        for(int kv = 0; kv < ATTN_BLOCK_KV; kv++) {
          ...
          // FA4 conditional online-softmax rescaling. Keeping the old origin
          // for small maximum changes is exact in real arithmetic because the
          // numerator and denominator retain the same common scale.
