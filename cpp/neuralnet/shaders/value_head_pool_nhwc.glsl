// Value-head global pooling on NHWC activations.
// input: [N, paddedSpatialSize, C]
// output: [N, 1, 3*C], represented as an FP32 feature vector at the boundary.
...
layout(binding = 2) readonly buffer MaskSumBuf { float data[]; } maskSumBuf;
...
void main() {
  ...
  if(c < pc.numChannels) {
    for(int xy = xyBase; xy < pc.paddedSpatialSize; xy += XYSTRIDE)
      sum += ldIn((n * pc.paddedSpatialSize + xy) * pc.numChannels + c);
  }
