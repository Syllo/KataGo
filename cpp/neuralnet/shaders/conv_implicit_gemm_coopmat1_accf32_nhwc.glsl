...
//      TIME. A warp issued 32 separate 2-byte loads covering 64 contiguous
...
//      halves = 144 bytes, so all 32 lanes of a warp hit the same handful of
//      with mm = tid % BM, i.e. unit stride -- conflict-free.
//
//      mm = tid % BM being thread-constant across the A_PER_THREAD s-loop
//      (BLOCK_SIZE is a multiple of BM), which lets SLAB_INSIDE_TAP hoist
...
//   * mm = i / (BK/8) is thread-constant across the s-loop again (BLOCK_SIZE
...
layout(binding = 0) readonly buffer InBufV8 { uvec4 data[]; } inBufV8;
layout(binding = 0) readonly buffer InBufH { float16_t data[]; } inBufH;
layout(binding = 1) readonly buffer FilterBufH { f16vec4 data[]; } filterBufV4H;
layout(binding = 2) buffer OutBufH { float16_t data[]; } outBufH;
...
layout(binding = 3) readonly buffer ScaleBufH { float16_t data[]; } scaleBufH;
layout(binding = 4) readonly buffer BiasBufH { float16_t data[]; } biasBufH;
...
void main() {
  ...
  const uint cmsPerRow = WM / TM;
  const uint cmsPerCol = WN / TN;
  ...
  // Accumulators: cmsPerRow x cmsPerCol fragments per warp.
  coopmat<ACCUM_TYPE, gl_ScopeSubgroup, TM, TN, gl_MatrixUseAccumulator> sums[(WM / TM) * (WN / TN)];
  ...
  const uint B_PER_THREAD_PACKED = (bPackedCount + BLOCK_SIZE - 1u) / BLOCK_SIZE;

  // --- A staging: vec8 NHWC im2col gather.
  //
  // Lane mapping: kk8 = i % (BK/8) selects which group of 8 channels, mm =
  // i / (BK/8) selects the spatial position. Because BLOCK_SIZE is a multiple
    ...
    if(SLAB_INSIDE_TAP != 0u) { \
      ...
      [[unroll]] for(uint s = 0; s < A_PER_THREAD_V8; s++) { \
        ...
        if(mm < BM) { \
          ...
          const bool valid = kValid && m < spatialReal && \
                             y >= 0 && y < pc.nnYLen && x >= 0 && x < pc.nnXLen; \
...
#undef LOAD_B_TO_REGS
  ...
  [[unroll]] for(uint cmRow = 0; cmRow < cmsPerRow; cmRow++) {
    [[unroll]] for(uint cmCol = 0; cmCol < cmsPerCol; cmCol++) {
      const uint s = cmRow * cmsPerCol + cmCol;
      const uint mTileBase = mIrLocal * BM + warp_r * WM + cmRow * TM;
