...
layout(binding = 7) writeonly buffer PartialsBuf { float data[]; } partialsBuf;
layout(binding = 8) writeonly buffer StatsBuf { float data[]; } statsBuf;
...
void main() {
  ...
  if (myPos < SEQ_LEN_SPEC) {
    ...
    const uint partialsRowBase =
      ( (bh * KV_CHUNK_COUNT + chunkId) * SEQ_LEN_SPEC + myPos) * V_HEAD_DIM;
