// GLSL Compute Shader: Winograd batched GEMM — FP16 with FP32 KHR cooperative-matrix accumulation (coopmat1).
...
// Workgroup / warp / fragment tile dimensions — all specialization constants.
...
void main() {
  ...
  // Accumulators: cmsPerRow x cmsPerCol FP32 fragments per warp.
  ...
  // the A/B global loads (ceil(count/BLOCK_SIZE)). Spec-const sized; glslang
  // allocates for the compile-time max.
  const uint A_PER_THREAD = (aCountV8 + BLOCK_SIZE - 1u) / BLOCK_SIZE;
  const uint B_PER_THREAD = (bCountV8 + BLOCK_SIZE - 1u) / BLOCK_SIZE;
  ...
  uvec4 bPrefetch[(BK * SHMEM_B_STRIDE_V8 + BLOCK_SIZE - 1u) / BLOCK_SIZE];
  ...
  for(uint block = 0; block < K_SPEC; block += BK) {
    ...
    [[unroll]] for(uint kOff = 0; kOff < BK; kOff += TK) {
      ...
      [[unroll]] for(uint cmCol = 0; cmCol < cmsPerCol; cmCol++) {
        ...
        const uint nBase = warp_c * WN + cmCol * TN;
    ...
    if(hasNext) {
      barrier();  // WAR: all warps done reading this block's shared before overwrite.
      COMMIT_A_TO_SHARED();
      COMMIT_B_TO_SHARED();
  ...
  [[unroll]] for(uint cmRow = 0; cmRow < cmsPerRow; cmRow++) {
    [[unroll]] for(uint cmCol = 0; cmCol < cmsPerCol; cmCol++) {
      ...
      coopmat<float16_t, gl_ScopeSubgroup, TM, TN, gl_MatrixUseAccumulator> outFrag =
        coopmat<float16_t, gl_ScopeSubgroup, TM, TN, gl_MatrixUseAccumulator>(sums[s]);
      coopMatStore(
        outFrag,
        cBufH.data,
