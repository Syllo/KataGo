// GLSL Compute Shader: Per-position RMSNorm for transformer layers
...
shared float partials[64];  // [WG_XY_SIZE * WG_C_SIZE]
...
void main() {
  ...
  int lid_xy = lid % WG_XY_SIZE;
  int lid_c = lid / WG_XY_SIZE;
  ...
  int pid = lid_xy * WG_C_SIZE + lid_c;
  partials[pid] = acc;
  ...
  barrier();
  float rms = inversesqrt(partials[lid_xy * WG_C_SIZE] / float(pc.numChannels) + pc.epsilon);
