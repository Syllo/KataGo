// GLSL Compute Shader: Spatial RMSNorm Pass 1 - partial sum of squares
// Ports OpenCLKernels::transformerSpatialRMSNormSumSq
// Dispatch: global(TILE_SIZE * numCHWWorkgroups, batchSize, 1)
...
inputBuf;
layout(binding = 1) readonly buffer MaskBuf {
  float data[];
}
maskBuf;
layout(binding = 2) writeonly buffer PartialsBuf {
  float data[];
}
partialsBuf;
...
inputBufH;
layout(binding = 4) readonly buffer MaskBufH {
  float16_t data[];
}
maskBufH;
