// GLSL Compute Shader: Strided batched GEMM — FP32 or FP16 storage + FP32 shared-memory tiles
...
// FP32 buffers use mat2x4/vec4 views. FP16 buffers use f16mat2x4/f16vec4 views,
// widened/narrowed at global I/O. Shared-memory tiles and accumulation are FP32 in both modes.
// M must be ÷8, NN=WG_Y*RN must be ÷4 (host guarantees both); K is unconstrained.
//
// Tiled microkernel: workgroup (WG_X × WG_Y), each thread owns 4(m) × RN(n).
...
bBufV4;
layout(binding = 2) buffer CBufV4 {
  vec4 data[];
}
cBufV4;
layout(binding = 3) readonly buffer ABufV8H {
  f16mat2x4 data[];
}
aBufV8H;
...
bBufV4H;
layout(binding = 5) buffer CBufV4H {
  f16vec4 data[];
}
cBufV4H;
...
mat2x4 ldA8(int base_v8, int k, int m8) {
  if(USE_FP16_STORAGE == 1) {
    f16mat2x4 v = aBufV8H.data[base_v8 + k * (pc.M / 8) + m8];
    return mat2x4(vec4(v[0]), vec4(v[1]));
  }
  return aBufV8.data[base_v8 + k * (pc.M / 8) + m8];
}
...
// +1 LDS padding to reduce bank conflicts (stagger row starts across banks).
const int MM = WG_X * 4;
const int MM4 = MM / 4;
const int MM8 = MM / 8;
const int NN = WG_Y * RN;
shared vec4 tileA[TILE_K * (MM4 + 1)];
...
void main() {
  ...
  for(int kBase = 0; kBase < K_SPEC; kBase += TILE_K) {
    ...
    for(int i = tid; i < TILE_K * MM8; i += T) {
      ...
      int gm8 = mBase / 8 + mm8;
      int mm4 = mm8 * 2;
      mat2x4 a8 = (gk < K_SPEC && gm8 * 8 < pc.M) ? ldA8(aBase_v8, gk, gm8) : mat2x4(vec4(0.0), vec4(0.0));
    ...
    for(int i = tid; i < TILE_K * (NN / 4); i += T) {
      ...
      int gk = kBase + kk;
      int gn4 = nBase / 4 + n4i;
      vec4 b4 = (gk < K_SPEC && gn4 * 4 < pc.N) ? ldB4(bBase_v4, gk, gn4) : vec4(0.0);
