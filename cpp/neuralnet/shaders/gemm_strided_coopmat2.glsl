// GLSL Compute Shader: Strided batched GEMM -- FP16 with NV cooperative matrix2.
...
void main() {
  ...
  if(ALIGNED != 0u && ic * BN + BN <= NReal)
    coopMatStoreTensorNV(
      matC, cBufH.data, cBase, sliceTensorLayoutNV(layoutC, ic * BN, BN, ir * BM, BM), transposeView);
  else
    coopMatStoreTensorNV(
      matC, cBufH.data, cBase, sliceTensorLayoutNV(layoutCClamp, ic * BN, BN, ir * BM, BM), transposeView);
}
