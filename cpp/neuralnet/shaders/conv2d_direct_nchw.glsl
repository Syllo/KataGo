// GLSL Compute Shader: Direct 2D convolution fallback (for non-3x3/5x5 sizes)
// Ports OpenCLKernels::conv2dNCHW
// input: [N, ic, paddedSpatialSize]   filter: [oc, ic, fy, fx]   output: [N, oc, paddedSpatialSize]
// Dispatch: global(ceil(nnXLen/TILE_XSIZE), ceil(nnYLen/TILE_YSIZE), numOutChannels)
...
layout(local_size_x_id = 0) in;  // TILE_XSIZE
layout(local_size_y_id = 1) in;  // TILE_YSIZE
      ...
      for(int fy = 0; fy < fySize; fy++) {
        ...
        for(int fx = 0; fx < fxSize; fx++) {
          ...
          if(ix < 0 || ix >= pc.nnXLen)
            continue;
          int inIdx = (n * pc.numInChannels + ic) * pc.paddedSpatialSize + iy * pc.nnXLen + ix;
          int filIdx = ((oc * pc.numInChannels + ic) * fySize + fy) * fxSize + fx;
          acc += ldIn(inIdx) * ldFilter(filIdx);
        }
      }
    }
    int outIdx = (n * pc.numOutChannels + oc) * pc.paddedSpatialSize + oy * pc.nnXLen + ox;
    stOut(outIdx, acc);
  }
}
