...
void main() {
  ...
  vec4 a = ldM4(i4);
  vec4 g = ldG4(i4);
  // exp2 maps directly to the base-2 exponential instruction on GPUs that
  // expose one. This is algebraically identical to exp(-a).
  vec4 silu_a = a / (vec4(1.0) + exp2(-a * vec4(1.4426950408889634)));
  stO4(i4, silu_a * g);
}
