...
void main() {
  ...
  for(int kBase = 0; kBase < K_SPEC; kBase += TILE_K) {
    ...
    if(USE_FP16_STORAGE == 1 && TILE_K % 4 == 0 && K_SPEC % 4 == 0) {
      ...
      for(int i = tid; i < TILE_K * WG_X; i += threadCount) {
        ...
        vec4 a = vec4(0.0);
        if(gk < K_SPEC) {
          if(gm0 + 0 < pc.M) a.x = ldA(aBase, gm0 + 0, gk);
          if(gm0 + 1 < pc.M) a.y = ldA(aBase, gm0 + 1, gk);
          if(gm0 + 2 < pc.M) a.z = ldA(aBase, gm0 + 2, gk);
          if(gm0 + 3 < pc.M) a.w = ldA(aBase, gm0 + 3, gk);
        }
  ...
  if(USE_FP16_STORAGE == 1 && RN % 4 == 0 && pc.N_real % 4 == 0) {
    ...
    for(int j = 0; j < RN; j++) {
      int n = myN + j;
      if(n < pc.N_real) {
        if(m0 + 0 < pc.M) stC(cBase, m0 + 0, n, acc[j].x);
        if(m0 + 1 < pc.M) stC(cBase, m0 + 1, n, acc[j].y);
        if(m0 + 2 < pc.M) stC(cBase, m0 + 2, n, acc[j].z);
        if(m0 + 3 < pc.M) stC(cBase, m0 + 3, n, acc[j].w);
      }
    }
  }
}
