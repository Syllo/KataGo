// GLSL Compute Shader: Spatial RMSNorm pass 3, NHWC layout.
...
layout(binding = 2) readonly buffer GammaBuf { float data[]; } gammaBuf;
layout(binding = 3) readonly buffer BetaBuf { float data[]; } betaBuf;
layout(binding = 4) readonly buffer MaskBuf { float data[]; } maskBuf;
layout(binding = 5) readonly buffer MaskSumBuf { float data[]; } maskSumBuf;
layout(binding = 6) readonly buffer SumSqBuf { float data[]; } sumSqBuf;
layout(binding = 7) readonly buffer InputBufH { float16_t data[]; } inputBufH;
...
void main() {
  ...
  float denom = maskSumBuf.data[n] * float(pc.numChannels);
  ...
  for(int e = 0; e < APPLY_ELTS_PER_THREAD; e++) {
    ...
    if(idx < totalElems) {
      ...
      int c = idx - xy * pc.numChannels;
      int bufIdx = n * totalElems + idx;
      float result = (ldIn(bufIdx) * rms * gammaBuf.data[c] + betaBuf.data[c]) *
        ldMask(n * pc.paddedSpatialSize + xy);
      stOut(bufIdx, result);
    }
  }
}
