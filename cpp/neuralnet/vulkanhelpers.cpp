...
namespace {
  ...
  struct VulkanGemmVariantFilter {
    ...
    bool allowDot2AccF16 = true;
    bool allowTiledFp16Compute = false;
  ...
  bool envValueTruthyLegacy(string_view v) {
    return !v.empty() && v != "0";
  }
  ...
  bool isSpecificGemmVariantToken(GemmVariantTokenKind kind) {
    return kind == GemmVariantTokenKind::Coopmat2AccF32 || kind == GemmVariantTokenKind::Coopmat2AccF16 ||
           ...
           kind == GemmVariantTokenKind::TiledFp16Compute;
  }

  bool isGemmFamilyToken(GemmVariantTokenKind kind) {
    return kind == GemmVariantTokenKind::Coopmat2 || kind == GemmVariantTokenKind::Coopmat1 ||
  ...
  void setGemmFamily(VulkanGemmVariantFilter& filter, GemmVariantTokenKind kind, bool enabled) {
    if(kind == GemmVariantTokenKind::Coopmat2) {
  ...
  void setAllGemmVariantMasks(VulkanGemmVariantFilter& filter, bool enabled) {
    ...
    filter.allowDot2AccF32 = enabled;
    filter.allowDot2AccF16 = enabled;
    filter.allowTiledFp16Compute = false;
  }
  ...
  void setSpecificGemmVariant(VulkanGemmVariantFilter& filter, GemmVariantTokenKind kind, bool enabled) {
    ...
    if(kind == GemmVariantTokenKind::Coopmat2AccF32)
      ...
      filter.allowDot2AccF16 = enabled;
    else if(kind == GemmVariantTokenKind::TiledFp16Compute)
      filter.allowTiledFp16Compute = false;
  }
  ...
  string makeGemmVariantFilterSignature(const VulkanGemmVariantFilter& filter) {
    if(!filter.configured)
      return "";
    ostringstream out;
    out << "cm2f32=" << (allowCoopmat2AccF32(filter) ? 1 : 0) << ",cm2f16=" << (allowCoopmat2AccF16(filter) ? 1 : 0)
        << ",cm1f32=" << (allowCoopmat1AccF32(filter) ? 1 : 0) << ",cm1f16=" << (allowCoopmat1AccF16(filter) ? 1 : 0)
        << ",dot2f32=" << (allowDot2AccF32(filter) ? 1 : 0) << ",dot2f16=" << (allowDot2AccF16(filter) ? 1 : 0)
        << ",tiledf16=" << (filter.allowAccF16 && filter.allowTiledFp16Compute ? 1 : 0);
    return out.str();
  }
  ...
  string validGemmVariantFilterTokens() {
    return "coopmat2, coopmat2accf32, coopmat2accf16, coopmat1, coopmat1accf32, coopmat1accf16, dot2, "
           "dot2accf32, dot2accf16, accf16, tiledaccf16";
  }

  VulkanGemmVariantFilter getVulkanGemmVariantFilter(Logger* logger) {
    VulkanGemmVariantFilter filter;

    string_view env = getenvStringView("KATAGO_VULKAN_GEMM_VARIANTS");
    ...
    if(!env.empty()) {
      ...
      for(string_view rawToken: rawTokens) {
        ...
        if(
          ...
          logVulkanEnvMessage(
            logger,
            "Warning: ignoring unrecognized token in KATAGO_VULKAN_GEMM_VARIANTS: " + string(parsed.original) +
    ...
    for(const ParsedGemmVariantToken& token: tokens) {
      switch(token.kind) {
        ...
        case GemmVariantTokenKind::TiledFp16Compute:
    ...
    if(envValueTruthyLegacy(getenvStringView("KATAGO_VULKAN_DISABLE_COOPMAT2"))) {
      filter.configured = true;
      filter.allowCoopmat2 = false;
    }

    if(filter.configured && logger != nullptr)
      logger->write(string("Vulkan: GEMM variant filter -> ") + gemmVariantFilterSummary(filter));

    return filter;
  }

  void applyVulkanGemmVariantFilter(VulkanDeviceInfo& info, const VulkanGemmVariantFilter& filter) {
    if(!(filter.allowAccF16 && filter.allowTiledFp16Compute)) {
      info.filterDisabledGemmTiledFp16Compute = info.supportsGemmTiledFp16Compute;
      info.supportsGemmTiledFp16Compute = false;
    }
...
vector<VulkanDeviceInfo> VulkanDeviceInfo::getAllDeviceInfosOnSystem(VkInstance instance, Logger* logger) {
  ...
  for(uint32_t i = 0; i < deviceCount; i++) {
    ...
    info.unfilteredSupportsGemmTiledFp16Compute = info.supportsGemmTiledFp16Compute;
...
namespace {
  ...
  static InitializedVulkanDevice* createDevice(
    ...
    dev->info.supportsGemmTiledFp16Compute = info.supportsGemmTiledFp16Compute && enableFP16Compute;
