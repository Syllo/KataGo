...
//
// When using FP16:
//   - Every "spatial" tensor (NCHW trunks, mask) is in FP16.
...
// Vulkan kernels expect spatial tensors in NCHW layout.
// Input extraction can produce NHWC, so convert in-place when requested.
...
namespace {
  ...
  bool reconcileCoopmatParams(VulkanTuneParams& cfg, const VulkanDeviceInfo& info, Logger* logger) {
    if(
      cfg.enableWinogradGemmCoopmat == 0 && cfg.enableGemmStridedCoopmat == 0 && cfg.enableWinogradGemmCoopmat2 == 0 &&
      cfg.enableGemmStridedCoopmat2 == 0 && cfg.enableWinogradGemmCoopmatAccF16 == 0 &&
      cfg.enableGemmStridedCoopmatAccF16 == 0 && cfg.enableWinogradGemmCoopmat2AccF16 == 0 &&
      cfg.enableGemmStridedCoopmat2AccF16 == 0)
      return false;
    ...
    if(cfg.enableWinogradGemmCoopmat != 0) {
      ...
      else if(cfg.coopmatWarp != (int32_t)info.subgroupSize)
        why = "coopmatWarp (" + Global::intToString(cfg.coopmatWarp) + ") does not match runtime subgroup size (" +
              Global::intToString((int)info.subgroupSize) + ")";
      else if(!shapeSupported(info.coopmatShapes, cfg.coopmatTM, cfg.coopmatTN, cfg.coopmatTK))
        why = "its coopmat fragment shape is not one this GPU reports";
      if(!why.empty()) {
        disable("winogradGemmCoopmat", why);
        cfg.enableWinogradGemmCoopmat = 0;
        disabledAny = true;
      }
    }
    if(cfg.enableGemmStridedCoopmat != 0) {
      string why;
      if(!info.supportsCoopmat1F16)
        why = "this GPU does not support cooperative matrices";
      else if(cfg.stridedCoopmatWarp != (int32_t)info.subgroupSize)
        why = "stridedCoopmatWarp (" + Global::intToString(cfg.stridedCoopmatWarp) +
              ") does not match runtime subgroup size (" + Global::intToString((int)info.subgroupSize) + ")";
      else if(!shapeSupported(info.coopmatShapes, cfg.stridedCoopmatTM, cfg.stridedCoopmatTN, cfg.stridedCoopmatTK))
        why = "its coopmat fragment shape is not one this GPU reports";
      if(!why.empty()) {
        disable("gemmStridedCoopmat", why);
        cfg.enableGemmStridedCoopmat = 0;
        disabledAny = true;
      }
    }
    ...
    if(cfg.enableGemmStridedCoopmat2 != 0) {
      string why;
      if(!info.supportsCoopmat2F16)
        why = "this GPU does not support cooperative matrix2";
      else if(!coopmat2TileSupported(
        ...
        why = "its coopmat2 flexible dimensions are not one this GPU reports";
      if(!why.empty()) {
        disable("gemmStridedCoopmat2", why);
        cfg.enableGemmStridedCoopmat2 = 0;
        disabledAny = true;
      }
    }
    if(cfg.enableWinogradGemmCoopmatAccF16 != 0) {
      ...
      else if(cfg.coopmatAccF16Warp != (int32_t)info.subgroupSize)
        why = "coopmatAccF16Warp (" + Global::intToString(cfg.coopmatAccF16Warp) +
              ") does not match runtime subgroup size (" + Global::intToString((int)info.subgroupSize) + ")";
      else if(!shapeSupported(info.coopmatAccF16Shapes, cfg.coopmatAccF16TM, cfg.coopmatAccF16TN, cfg.coopmatAccF16TK))
        why = "its coopmatAccF16 fragment shape is not one this GPU reports";
      if(!why.empty()) {
        disable("winogradGemmCoopmatAccF16", why);
        cfg.enableWinogradGemmCoopmatAccF16 = 0;
        disabledAny = true;
      }
    }
    if(cfg.enableGemmStridedCoopmatAccF16 != 0) {
      string why;
      if(!info.supportsCoopmat1F16AccF16)
        why = "this GPU does not support cooperative matrices with FP16 accumulation";
      else if(cfg.stridedCoopmatAccF16Warp != (int32_t)info.subgroupSize)
        why = "stridedCoopmatAccF16Warp (" + Global::intToString(cfg.stridedCoopmatAccF16Warp) +
              ") does not match runtime subgroup size (" + Global::intToString((int)info.subgroupSize) + ")";
      else if(!shapeSupported(
                info.coopmatAccF16Shapes,
                cfg.stridedCoopmatAccF16TM,
                cfg.stridedCoopmatAccF16TN,
                cfg.stridedCoopmatAccF16TK))
        why = "its coopmatAccF16 fragment shape is not one this GPU reports";
      if(!why.empty()) {
        disable("gemmStridedCoopmatAccF16", why);
        cfg.enableGemmStridedCoopmatAccF16 = 0;
        disabledAny = true;
      }
    }
    ...
    if(cfg.enableGemmStridedCoopmat2AccF16 != 0) {
      string why;
      if(!info.supportsCoopmat2F16AccF16)
        why = "this GPU does not support cooperative matrix2 with FP16 accumulation";
      else if(!coopmat2TileSupported(
        ...
        why = "its coopmat2AccF16 flexible dimensions are not one this GPU reports";
      if(!why.empty()) {
        disable("gemmStridedCoopmat2AccF16", why);
        cfg.enableGemmStridedCoopmat2AccF16 = 0;
        disabledAny = true;
      }
    }
...
struct ComputeContext {
  ...
  ComputeContext(
    ...
    for(auto& dev: devicesContext->devicesToUse) {
      ...
      //   3. modelDesc null: leave at compiled-in defaults silently (e.g. layer tests, the
      //      standalone tuner about to overwrite them anyway).
      ...
      auto requestGemmVariantsTuneForDevice = [&]() {
        if(modelDescForTuneLoad == nullptr)
          return;
        if(
          std::find(gpuIdxsNeedingGemmVariantsTune.begin(), gpuIdxsNeedingGemmVariantsTune.end(), dev->info.gpuIdx) ==
          gpuIdxsNeedingGemmVariantsTune.end())
          gpuIdxsNeedingGemmVariantsTune.push_back(dev->info.gpuIdx);
      };
      auto maybeRequestWinogradTransformsRetune = [&](const VulkanTuneParams& loaded) {
        if(modelDescForTuneLoad == nullptr)
          return;
        if(VulkanTuner::winogradTransformsTunedForCurrentLayout(loaded, fp16Storage))
          return;
        if(logger != nullptr)
          logger->write(
            "Vulkan tune file Winograd transforms were tuned for a different Winograd GEMM layout; re-tuning only "
            "winogradTransform and winogradBNActTransform.");
      ...
      if(!vulkanTunerFile.empty()) {
        ...
        try {
          ...
          if(!loaded.isValid())
            ...
            if(loaded.normalizeGemmVariantSelectionsForDevice(dev->info, fp16Storage, fp16Compute) && logger != nullptr)
              logger->write(
                "Vulkan tune file GEMM variant selection was normalized using tuned candidate order and current "
                "hardware filter: " +
                vulkanTunerFile);
        ...
        if(loadedExplicitFile) {
          ...
          if(!requestedFullRetune && !VulkanTuner::gemmVariantFilterTunedForCurrentHardware(loaded, dev->info))
            requestGemmVariantsTuneForDevice();
        ...
        if(!tryLoadFromCache(
          ...
          if(!VulkanTuner::gemmVariantFilterTunedForCurrentHardware(perDeviceParams, dev->info))
            requestGemmVariantsTuneForDevice();
    ...
    auto attempt = [&](int xLen, int yLen) -> bool {
      ...
      try {
        ...
        if(loaded.normalizeGemmVariantSelectionsForDevice(deviceInfo, fp16, fp16Compute) && logger != nullptr)
          logger->write(
            "Vulkan tune file GEMM variant selection was normalized using tuned candidate order and current hardware "
            "filter: " +
            path);
...
void NeuralNet::tuneVulkanComputeContext(
  ...
  bool verboseTuner,
  const std::string& saveFileOverride) {
  ctx->tuneSelf(modelDesc, batchSize, winograd3x3OutTile, benchIters, verboseTuner, saveFileOverride);
}
...
void ComputeContext::tuneSelf(
  ...
  for(int gpuIdx: toTune) {
    ...
    if(logger != nullptr) {
      std::ifstream probe;
      if(FileUtils::tryOpen(probe, outputPath))
        logger->write("Re-tuning from scratch (ignoring existing " + outputPath + ")");
    }
    ...
    initialParams.winograd3x3OutTile = winograd3x3OutTile;
    winograd3x3OutTileFor(initialParams);

    VulkanTuneParams results;
    VulkanTuner::tune(
      &session, initialParams, modelDesc, batchSize, nnXLen, nnYLen, benchIters, std::cerr, verboseTuner, results);
    session.destroy();
...
void ComputeContext::tuneGemmVariantsSelf(
  ...
  for(int gpuIdx: toTune) {
    ...
    if(alreadyTuned != tunedParamsByKey.end()) {
      ...
      pipelines->tuneParams = alreadyTuned->second;
      if(
        !VulkanTuner::winogradTransformsTunedForCurrentLayout(pipelines->tuneParams, pipelines->usingFP16Storage) &&
    ...
    if(changed) {
      pipelines->tuneParams = results;
      if(
        !VulkanTuner::winogradTransformsTunedForCurrentLayout(results, pipelines->usingFP16Storage) &&
        std::find(gpuIdxsNeedingWinogradTransformsTune.begin(), gpuIdxsNeedingWinogradTransformsTune.end(), gpuIdx) ==
          gpuIdxsNeedingWinogradTransformsTune.end())
        gpuIdxsNeedingWinogradTransformsTune.push_back(gpuIdx);
...
void ComputeContext::tuneWinogradTransformsSelf(
  ...
  for(int gpuIdx: toTune) {
    ...
    CompiledPipelines* pipelines = compiledPipelinesByDevice.at(dev->info.physicalDevice);
    if(VulkanTuner::winogradTransformsTunedForCurrentLayout(pipelines->tuneParams, pipelines->usingFP16Storage)) {
    ...
    if(logger != nullptr) {
      logger->write(
        "Re-tuning Vulkan Winograd transforms for selected Winograd GEMM layout on " + gpuName + " for batch " +
    ...
    VulkanTuneParams results = pipelines->tuneParams;
    bool retuned = VulkanTuner::retuneWinogradTransformsForCurrentLayout(
      session, modelDesc, batchSize, nnXLen, nnYLen, benchIters, std::cerr, verboseTuner, results);
    session.destroy();
...
struct Buffers {
  ...
  Buffers(
    ...
    inputElts = static_cast<size_t>(model.numInputChannels) * B * paddedSpatialSize;
...
struct Model {
  ...
  // input, sumChannels reduces it to a per-batch maskSum.
  VkDevice device;
  ComputeKernel extractChannel0Kernel;
  ComputeKernel sumChannelsKernel;

  Model(const VulkanLayerContext& ctx, const ModelDesc* desc, int nnX, int nnY, int paddedSpatialSize, bool useFP16)
    : name(desc->name),
      ...
      device(ctx.device) {
    trunk = std::make_unique<TrunkVk>(ctx, &desc->trunk, nnX, nnY, paddedSpatialSize, useFP16);
    policyHead = std::make_unique<PolicyHeadVk>(ctx, &desc->policyHead, nnX, nnY, paddedSpatialSize, useFP16);
    valueHead = std::make_unique<ValueHeadVk>(ctx, &desc->valueHead, nnX, nnY, paddedSpatialSize, useFP16);
    extractChannel0Kernel = VulkanKernels::ExtractChannel0::build(ctx.device, ctx.pipelineCache, useFP16);
    sumChannelsKernel = VulkanKernels::SumChannels::build(ctx.device, ctx.pipelineCache, useFP16);
  }

  ~Model() {
    extractChannel0Kernel.destroy(device);
    sumChannelsKernel.destroy(device);
  }
  ...
  void setupMask(
    ...
    {
      VulkanKernels::ExtractChannel0::PC pc = {numSpatialFeatures, paddedSpatialSize};
      VulkanKernels::ExtractChannel0::dispatch(ctx, extractChannel0Kernel, input, mask, pc, maxBatchSize);
      cmdComputeBarrier(ctx.cmd, mask->buffer);
      // mark output mask of extractChannel0 input dependence for sumChannels and the BN/gpool consumers
    }
    {
      VulkanKernels::SumChannels::PC pc = {paddedSpatialSize, maxBatchSize};
      VulkanKernels::SumChannels::dispatch(ctx, sumChannelsKernel, mask, maskSum, pc, maxBatchSize);
      cmdComputeBarrier(ctx.cmd, maskSum->buffer);
      // mark output maskSum of sumChannels input dependence for gpool/valueHeadPool/spatialRMSNorm
    }
  }
  ...
  void dispatch(
    ...
    trunk->dispatch(ctx, scratch, input, inputGlobal, inputMeta, trunkBuf, mask, maskSum, maxBatchSize);
    // trunk->dispatch barriers its own trunkBuf output (trunk tip), so the heads read it directly.
    policyHead->dispatch(ctx, scratch, trunkBuf, mask, maskSum, policyPass, policy, maxBatchSize);
    valueHead->dispatch(ctx, scratch, trunkBuf, mask, maskSum, value, scoreValue, ownership, maxBatchSize);
...
struct ComputeHandle {
  ...
  ComputeHandle(
    ...
    model = std::make_unique<Model>(layerCtx, &lm->modelDesc, nnXLen, nnYLen, paddedSpatialSize, usingFP16Storage);
    ...
    if(DispatchProfiler::envEnabled()) {
      // ~1024 dispatch pairs covers the deepest networks (b40 trunk ~600 dispatches).
      profiler.init(device, timestampPeriodNs, computeQueueSupportsTimestamps, 1024);
    }
...
namespace {
  ...
  struct VulkanOutputRun {
    ...
    void prepareInputs() {
      const bool nhwc = gpuHandle->inputsUseNHWC;
      const int C = numSpatialFeatures, H = nnYLen, W = nnXLen;
      auto convertNCHW = [C, H, W, nhwc](float* rowSpatialInput) {
        if(!nhwc || C <= 0 || H <= 0 || W <= 0)
          return;
        ...
        if(totalSize <= 1)
          return;
        const int HW = H * W;
        auto targetIdx = [C, W, HW](int nhwcIdx) {
          int c = nhwcIdx % C, temp = nhwcIdx / C, x = temp % W, y = temp / W;
          return (c * HW) + (y * W) + x;
        };
        std::vector<uint8_t> processed(static_cast<size_t>(totalSize), 0);
        for(int i = 0; i < totalSize; i++) {
          if(processed[static_cast<size_t>(i)] != 0)
            continue;
          int t = targetIdx(i);
          if(t == i) {
            processed[static_cast<size_t>(i)] = 1;
            continue;
          }
          int cur = i;
          float inHand = rowSpatialInput[i];
          while(true) {
            int dst = targetIdx(cur);
            float at = rowSpatialInput[dst];
            rowSpatialInput[dst] = inHand;
            processed[static_cast<size_t>(dst)] = 1;
            inHand = at;
            cur = dst;
            if(cur == i)
              break;
          ...
          for(int i = 0; i < nnXYLen; i++) {
            if(maskRow[i] != 1.0f)
              throw StringError(
                ...
                " (value=" + Global::floatToString(maskRow[i]) +
    ...
    void copyInputsToMappedBuffers() {
      const size_t spatialUploadBytes = static_cast<size_t>(numSpatialFeatures) * batchSize * paddedSpatialSize *
    ...
    void writeOutputsToResults() {
      ...
      for(int nIdx = 0; nIdx < batchSize; nIdx++) {
        ...
        if(numPolicyChannels == 2 || (numPolicyChannels == 4 && modelVersion >= 16)) {
          ...
          for(int i = 0; i < nnXYLen; i++)
            policyProbsTmp[i] = blend(policySrc[i], policySrc[i + paddedSpatialSize]);
...
namespace {
  ...
  // Download + convert-to-FP32 + unpads from paddedLen back to xyLen.
  void downloadTestBuf(
    VulkanWorkContext& ctx,
    VulkanBuffer* srcBuf,
    vector<float>& dst,
    size_t numElts,
    ...
    if(!fp16) {
      ...
      if(paddedLen == xyLen) {
        memcpy(dst.data(), src, numElts * sizeof(float));
      } else {
        size_t numChannelGroups = numElts / xyLen;
        for(size_t c = 0; c < numChannelGroups; c++)
          for(int xy = 0; xy < xyLen; xy++)
            dst[c * xyLen + xy] = src[c * paddedLen + xy];
      }
    } else {
      half_t* src = static_cast<half_t*>(mapped);
      if(paddedLen == xyLen) {
        for(size_t i = 0; i < numElts; i++)
          dst[i] = (float)src[i];
      } else {
        ...
        for(size_t c = 0; c < numChannelGroups; c++)
          for(int xy = 0; xy < xyLen; xy++)
            dst[c * xyLen + xy] = (float)src[c * paddedLen + xy];
      }
    }
    vkUnmapMemory(ctx.dev->device, staging.memory);
  }
...
bool NeuralNet::testEvaluateConv(
  ...
  auto inputBuf =
    makeTestSpatialInputBuf(ctx, inputBuffer, batchSize, desc->inChannels, nnXYLen, paddedSpatialSize, fp16);
  ...
  downloadTestBuf(ctx, outputBuf.get(), outputBuffer, outElts, nnXLen * nnYLen, paddedSpatialSize, fp16);
  return true;
}
...
bool NeuralNet::testEvaluateBatchNorm(
  ...
  auto inputBuf =
    makeTestSpatialInputBuf(ctx, inputBuffer, batchSize, desc->numChannels, nnXYLen, paddedSpatialSize, fp16);
  ...
  ctx.beginRecording();
  layer.dispatch(CmdCtx{ctx.cmd, pushFn}, inputBuf.get(), outputBuf.get(), maskBuf.get(), paddedSpatialSize, batchSize);
  ctx.submitAndWait();

  downloadTestBuf(ctx, outputBuf.get(), outputBuffer, elts, nnXLen * nnYLen, paddedSpatialSize, fp16);
  return true;
}
...
bool NeuralNet::testEvaluateResidualBlock(
  ...
  auto trunkBuf = makeTestSpatialInputBuf(ctx, outputBuffer, batchSize, trunkC, nnXYLen, paddedSpatialSize, fp16);
  ...
  downloadTestBuf(ctx, trunkBuf.get(), outputBuffer, trunkElts, nnXLen * nnYLen, paddedSpatialSize, fp16);
  return true;
}
