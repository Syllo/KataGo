...
// no subgroup/fragment tiling constants. Slot 17 selects the hoisted gather when a
...
pc;
layout(binding = 0) readonly buffer InV8 {
  uvec4 data[];
}
inV8;
layout(binding = 1) readonly buffer Filter {
  float16_t data[];
}
filterBuf;
...
outBuf;
layout(binding = 3) readonly buffer Unused3 {
  float16_t data[];
}
unused3;
layout(binding = 4) readonly buffer Unused4 {
  float16_t data[];
}
unused4;
layout(binding = 5) readonly buffer Unused5 {
  float16_t data[];
}
unused5;
...
void main() {
  ...
  // WORKGROUP_SIZE % (BK/8) == 0 is host-enforced so mmStep tiles [0,BM) exactly.
  ...
  if(ADD_TO_OUTPUT != 0u) {
    ...
    coopMatLoadTensorNV(old, outBuf.data, cBase, sliceTensorLayoutNV(layoutC, int(ir * BM), BM, int(ic * BN), BN));
...
#define COMMIT_A_TO_SHARED() \
  ...
  [[dont_unroll]] for(uint blk = 0u; blk < K_SPEC; blk += BK) {
    ...
    coopMatLoadTensorNV(
      b,
      filterBuf.data,
      bBase + (ic * kBlocks + blk / BK) * BN * bStride,
      sliceTensorLayoutNV(layoutB, 0, BN, 0, BK),
      transposeView);
...
#undef LOAD_A_TO_PREFETCH
  ...
  coopMatStoreTensorNV(result, outBuf.data, cBase, sliceTensorLayoutNV(layoutC, int(ir * BM), BM, int(ic * BN), BN));
}
