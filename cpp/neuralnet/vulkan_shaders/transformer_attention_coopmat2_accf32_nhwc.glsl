...
float smearReduce(const in float x, const in float y) {
  return x;
}
float maxPerElement(const in uint32_t row, const in uint32_t col, const in float x, const in float y) {
  return max(x, y);
}
float exp2PerElement(const in uint32_t row, const in uint32_t col, const in float x) {
  return exp2(x);
}
float inversePerElement(const in uint32_t row, const in uint32_t col, const in float x) {
  return x > 0.0 ? 1.0 / x : 0.0;
}

float maskScore(
  const in uint32_t row,
  const in uint32_t col,
  const in float x,
  const in uint32_t n,
  const in uint32_t qStart,
  const in uint32_t kvStart) {
  const uint qPos = qStart + row, kvPos = kvStart + col;
  return qPos < SEQ_LEN_SPEC && kvPos < SEQ_LEN_SPEC && qMask[row] != 0u && kvMask[col] != 0u ? x : NEG_BIG;
}
...
float maskedExp2(
  ...
  const in uint32_t kvStart) {
  const uint qPos = qStart + row, kvPos = kvStart + col;
  return qPos < SEQ_LEN_SPEC && kvPos < SEQ_LEN_SPEC && qMask[row] != 0u && kvMask[col] != 0u ? exp2(x) : 0.0;
}
...
float16_t ropeLoad(const in RopePair raw, const in uint blockCoords[2], const in uint coordInBlock[2]) {
  ...
  const uint tableIdx =
    LEARNABLE_ROPE != 0u ? (kvh * SEQ_LEN_SPEC + blockCoords[0]) * pairs + pairIdx : blockCoords[0] * pairs + pairIdx;
...
void main() {
  ...
  // loop so BLOCK_Q can be tuned independently of WORKGROUP_SIZE.
  for(uint row = tid; row < BLOCK_Q; row += WORKGROUP_SIZE) {
  ...
  tensorLayoutNV<2, gl_CooperativeMatrixClampModeConstantNV> qLayoutClamp =
    createTensorLayoutNV(2, gl_CooperativeMatrixClampModeConstantNV);
  if(APPLY_ROPE != 0u)
    qLayout = setTensorLayoutBlockSizeNV(qLayout, 1u, 2u);
  qLayout = setTensorLayoutDimensionNV(qLayout, SEQ_LEN_SPEC, uint(pc.numHeads) * HEAD_DIM);
  qLayout = setTensorLayoutStrideNV(
    qLayout, (APPLY_ROPE != 0u ? uint(pc.numHeads) * HEAD_DIM / 2u : uint(pc.numHeads) * HEAD_DIM), 1u);
  if(APPLY_ROPE != 0u)
    qLayoutClamp = setTensorLayoutBlockSizeNV(qLayoutClamp, 1u, 2u);
  qLayoutClamp = setTensorLayoutDimensionNV(qLayoutClamp, SEQ_LEN_SPEC, uint(pc.numHeads) * HEAD_DIM);
  qLayoutClamp = setTensorLayoutStrideNV(
    qLayoutClamp, (APPLY_ROPE != 0u ? uint(pc.numHeads) * HEAD_DIM / 2u : uint(pc.numHeads) * HEAD_DIM), 1u);
  ...
  tensorLayoutNV<2, gl_CooperativeMatrixClampModeConstantNV> kLayoutClamp =
    createTensorLayoutNV(2, gl_CooperativeMatrixClampModeConstantNV);
  if(APPLY_ROPE != 0u)
    kLayout = setTensorLayoutBlockSizeNV(kLayout, 1u, 2u);
  kLayout = setTensorLayoutDimensionNV(kLayout, SEQ_LEN_SPEC, uint(pc.numKVHeads) * HEAD_DIM);
  kLayout = setTensorLayoutStrideNV(
    kLayout, (APPLY_ROPE != 0u ? uint(pc.numKVHeads) * HEAD_DIM / 2u : uint(pc.numKVHeads) * HEAD_DIM), 1u);
  if(APPLY_ROPE != 0u)
    kLayoutClamp = setTensorLayoutBlockSizeNV(kLayoutClamp, 1u, 2u);
  kLayoutClamp = setTensorLayoutDimensionNV(kLayoutClamp, SEQ_LEN_SPEC, uint(pc.numKVHeads) * HEAD_DIM);
  kLayoutClamp = setTensorLayoutStrideNV(
    kLayoutClamp, (APPLY_ROPE != 0u ? uint(pc.numKVHeads) * HEAD_DIM / 2u : uint(pc.numKVHeads) * HEAD_DIM), 1u);
  ...
  if(ALIGNED_TILES != 0u) {
    if(APPLY_ROPE != 0u)
      coopMatLoadTensorNV(
        q, qBuf.data, qBase, sliceTensorLayoutNV(qLayout, qStart, BLOCK_Q, h * HEAD_DIM, HEAD_DIM), ropeLoad);
    else
      coopMatLoadTensorNV(q, qBuf.data, qBase, sliceTensorLayoutNV(qLayout, qStart, BLOCK_Q, h * HEAD_DIM, HEAD_DIM));
  } else {
    if(APPLY_ROPE != 0u)
      coopMatLoadTensorNV(
        q, qBuf.data, qBase, sliceTensorLayoutNV(qLayoutClamp, qStart, BLOCK_Q, h * HEAD_DIM, HEAD_DIM), ropeLoad);
    else
      coopMatLoadTensorNV(
        q, qBuf.data, qBase, sliceTensorLayoutNV(qLayoutClamp, qStart, BLOCK_Q, h * HEAD_DIM, HEAD_DIM));
  }
  ...
  for(uint kvStart = 0u; kvStart < SEQ_LEN_SPEC; kvStart += BLOCK_KV) {
    for(uint col = tid; col < BLOCK_KV; col += WORKGROUP_SIZE) {
    ...
    coopmat<float, gl_ScopeWorkgroup, BLOCK_Q, BLOCK_KV, gl_MatrixUseAccumulator> scores =
      coopmat<float, gl_ScopeWorkgroup, BLOCK_Q, BLOCK_KV, gl_MatrixUseAccumulator>(0.0);
    ...
    if(ALIGNED_TILES != 0u) {
      if(APPLY_ROPE != 0u)
        coopMatLoadTensorNV(
          ...
          kBuf.data,
          kBase,
          sliceTensorLayoutNV(kLayout, kvStart, BLOCK_KV, kvh * HEAD_DIM, HEAD_DIM),
          transposeView,
          ropeLoad);
      else
        coopMatLoadTensorNV(
      ...
      if(APPLY_ROPE != 0u)
        coopMatLoadTensorNV(
          ...
          kBuf.data,
          kBase,
          sliceTensorLayoutNV(kLayoutClamp, kvStart, BLOCK_KV, kvh * HEAD_DIM, HEAD_DIM),
          transposeView,
          ropeLoad);
      else
        coopMatLoadTensorNV(
    ...
    if(ALIGNED_TILES != 0u)
      coopMatLoadTensorNV(
        values, vBuf.data, vBase, sliceTensorLayoutNV(vLayout, kvStart, BLOCK_KV, kvh * V_HEAD_DIM, V_HEAD_DIM));
    else
      coopMatLoadTensorNV(
        values, vBuf.data, vBase, sliceTensorLayoutNV(vLayoutClamp, kvStart, BLOCK_KV, kvh * V_HEAD_DIM, V_HEAD_DIM));
  ...
  if(ALIGNED_TILES != 0u)
    coopMatStoreTensorNV(
      outputMatrix, outBuf.data, outBase, sliceTensorLayoutNV(outLayout, qStart, BLOCK_Q, h * V_HEAD_DIM, V_HEAD_DIM));
  else
    coopMatStoreTensorNV(
      outputMatrix,
      outBuf.data,
