...
shared float16_t coopStage[(WORKGROUP_SIZE / SUBGROUP_SIZE) * TM * TN];

void storeC(uint i, float16_t v) {
  if(ADD_TO_OUTPUT != 0u)
    v += cBufH.data[i];
  cBufH.data[i] = v;
}
  ...
  [[unroll]] for(uint r = 0u; r < cmsPerRow; r++) [[unroll]]
    for(uint c = 0u; c < cmsPerCol; c++) {
      ...
      if(m0 + TM <= uint(pc.M) && n0 + TN <= uint(pc.N_real)) {
        ...
        coopMatStore(
          result, cBufH.data, cBase + m0 * uint(pc.N_real) + n0, uint(pc.N_real), gl_CooperativeMatrixLayoutRowMajor);
      } else {
