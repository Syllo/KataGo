...
float maintenanceRowMax(const in float a, const in float b) {
  return max(a, b);
}
float maintenanceRowSum(const in float a, const in float b) {
  return a + b;
}
...
float maintenanceExpScore(const in uint row, const in uint col, const in float value) {
  ...
  return maintenanceRowActive[globalRow] != 0.0 && kvMask[globalCol] != 0.0
           ? exp2(value - maintenanceRowOrigin[globalRow])
           : 0.0;
}
...
void main() {
  ...
  for(uint kvStart = 0u; kvStart < SEQ_LEN_SPEC; kvStart += BLOCK_KV) {
    ...
    if(anyActive) {
      ...
      coopmat<float, gl_ScopeSubgroup, TM, TN, gl_MatrixUseAccumulator> scoreFrag[STRIPS_PER_SUBGROUP * (BLOCK_KV / TN)];
      coopmat<float, gl_ScopeSubgroup, TM, TN, gl_MatrixUseAccumulator> probFrag[STRIPS_PER_SUBGROUP * (BLOCK_KV / TN)];
      ...
      [[unroll]] for(uint c = 0u; c < BLOCK_KV / TN; c++) {
        [[unroll]] for(uint s = 0u; s < STRIPS_PER_SUBGROUP; s++) {
          ...
          coopMatReduceEXT(
            rowMaxFrag, scoreFrag[s * (BLOCK_KV / TN) + c], gl_CooperativeMatrixReduceRowEXT, maintenanceRowMax);
      ...
      [[unroll]] for(uint c = 0u; c < BLOCK_KV / TN; c++) {
        [[unroll]] for(uint s = 0u; s < STRIPS_PER_SUBGROUP; s++) {
          ...
          maintenanceRowBase = qBase;
          maintenanceColBase = c * TN;
          coopMatPerElementEXT(
            probFrag[s * (BLOCK_KV / TN) + c], scoreFrag[s * (BLOCK_KV / TN) + c], maintenanceExpScore);
          coopmat<float, gl_ScopeSubgroup, TM, TN, gl_MatrixUseAccumulator> rowSumFrag;
          coopMatReduceEXT(
            rowSumFrag, probFrag[s * (BLOCK_KV / TN) + c], gl_CooperativeMatrixReduceRowEXT, maintenanceRowSum);
      ...
      [[unroll]] for(uint s = 0u; s < STRIPS_PER_SUBGROUP; s++) {
        ...
        [[unroll]] for(uint k = 0u; k < BLOCK_KV / TK; k++) {
          coopmat<float16_t, gl_ScopeSubgroup, TM, TK, gl_MatrixUseA> pf =
            coopmat<float16_t, gl_ScopeSubgroup, TM, TK, gl_MatrixUseA>(probFrag[s * (BLOCK_KV / TN) + k]);
