// GLSL Compute Shader: Winograd batched GEMM — FP16 with DOT2 (OpFDot2MixAcc16VALVE)
...
// Subgroup-level register tiling (ggml mul_mm.comp shape) with the accumulation done
// by the native v_dot2 instruction: each dot_product() folds an f16vec4·f16vec4
// (4 K-values) into an FP16 accumulator with two v_dot2 calls.
//
// Memory layout note:
...
// OpFDot2MixAcc16VALVE: f16vec2·f16vec2 + f16 → f16
...
// Workgroup / subgroup tile dimensions — all specialization constants.
...
void main() {
  ...
  // Accumulator: SGMITER*TM rows × SGNITER*TN cols as f16vec2 pairs.
  ...
  const uint A_PER_THREAD = (aCount + WORKGROUP_SIZE - 1u) / WORKGROUP_SIZE;
  ...
  f16vec4 aPrefetch[(BM * (BK / 4u) + WORKGROUP_SIZE - 1u) / WORKGROUP_SIZE];
  f16vec4 bPrefetch[(BK * (BN / 4u) + WORKGROUP_SIZE - 1u) / WORKGROUP_SIZE];
